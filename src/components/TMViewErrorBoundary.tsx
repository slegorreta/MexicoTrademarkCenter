...
export default class TMViewErrorBoundary extends Component<Props, State> {
  ...
  componentDidCatch(error: Error, info: ErrorInfo) {
    console.error('[ClearancePanel] Error boundary caught:', error, info.componentStack);
  }
  ...
  render() {
    ...
    const isEn = lang === 'en';
    const isClearance = this.props.mode === 'clearance';

    const title = isClearance
      ? (isEn ? 'Could not display the full report' : 'No se pudo mostrar el reporte completo')
      : (isEn ? 'TMView data unavailable' : 'Datos de TMView no disponibles');

    const body = isClearance
      ? (isEn
          ? 'An error occurred while rendering the clearance report. Please retry — your analysis was completed successfully.'
          : 'Ocurrió un error al mostrar el reporte de búsqueda. Por favor reintente — su análisis se completó correctamente.')
      : (isEn
