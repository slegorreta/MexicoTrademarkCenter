...
async function buildPdf(
  ...
  // ── PAGE 1: Cover ──────────────────────────────────────────────────────────
  {
    ...
    p.drawText(markName.slice(0, 60), { x: MARGIN + 16, y: y - 18, size: 20, font: bold, color: C.darkGreen });
    y -= 72;
  ...
  // ── PAGE 7: LFPPI Registrability ──────────────────────────────────────────
  {
    ...
    y -= 8;
    p.drawText("Evaluation against Mexico's Ley Federal de Protección a la Propiedad Industrial (LFPPI)", { x: MARGIN, y, size: 8, font: regular, color: C.gray });
    y -= 24;
  ...
  // ── PAGE 8: Conflicting Registrations (MARCia) ────────────────────────────
  {
    ...
    if (findings.length === 0) {
      ...
      for (let i = 0; i < findings.length && y > 80; i++) {
        ...
        p.drawText(f.name.slice(0, 32), { x: MARGIN + 6, y: y - 8, size: 8, font: isExact ? bold : regular, color: isExact ? C.red : C.black });
        p.drawText(f.classNum.slice(0, 8), { x: MARGIN + 230, y: y - 8, size: 8, font: regular, color: C.black });
        p.drawText(f.status.slice(0, 14), { x: MARGIN + 280, y: y - 8, size: 8, font: regular, color: C.black });
        p.drawText(f.holder.slice(0, 22), { x: MARGIN + 360, y: y - 8, size: 8, font: regular, color: C.black });
        y -= 20;
      }
    }
  }
  ...
  // ── PAGE 10: Domain Availability ──────────────────────────────────────────
  {
    ...
    if (domains.length === 0) {
      ...
      for (let i = 0; i < domains.length && y > 80; i++) {
        ...
        p.drawText(d.domain, { x: MARGIN + 6, y: y - 8, size: 9, font: regular, color: C.black });
